...
## <code>EXPLAIN</code> output format {#code-explain-code-output-format}
...
-   MySQL returns the query plan that is **being executing**, while TiDB returns the <strong>last executed</strong> query plan.
-   MySQL requires the login user to be the same as the connection being queried, or the login user has the **`PROCESS`** privilege; while TiDB requires the login user to be the same as the connection being queried, or the login user has the <strong><code>SUPER</code></strong> privilege.
